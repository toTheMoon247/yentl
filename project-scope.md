...
Used exclusively by matchmakers.
...
| Layer              | Pick                           | Why                                                 |
| ------------------ | ------------------------------ | --------------------------------------------------- |
| Dating App         | Swift + SwiftUI                | Native iOS experience                               |
| Yentl App          | Swift + SwiftUI                | Dedicated internal matchmaking application          |
...
The interface is intentionally designed to feel similar to the Dating App.
...
The full public profile opens exactly as it appears inside the Dating App.
...
The following information is only visible inside the Yentl App:
...
The boost temporarily increases profile visibility inside the Dating App.
...
**The Dating App** is what regular users see. It looks and feels like a normal dating app — create a profile, upload photos, swipe on others, and chat after a match. The difference is that the matches you receive have been hand-picked by a real matchmaker, not just generated by an algorithm.

**The Yentl App** is the private internal tool our matchmakers use. Regular users never see it. Matchmakers use it to review new sign-ups, look at users waiting in the matchmaking queue, and create matches between compatible people. The interface is designed to feel like a swipe-based dating app rather than a spreadsheet, so matchmaking is fast and human.
